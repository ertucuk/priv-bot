...
module.exports = async function memberRoleUpdate(client, guild, audit, member, changes) {
  ...
  if (safeMode && !safeMode.isWarn) await punish(client, member, safeMode.punishType, 'Üyenin Rolleri Güncellendi!');
  else if (!safeMode) await punish(client, member, 3, 'Üyenin Rolleri Güncellendi!');
};